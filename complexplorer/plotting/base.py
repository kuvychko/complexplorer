...
class BasePlotter(ABC):
    ...
    def _prepare_data(self) -> dict[str, np.ndarray]:
        ...
        # Cache data
        self._data = {"z": self.z, "f": self.f, "x": x, "y": y, "u": u, "v": v, "colors": colors}

        return self._data

    def _get_plot_limits(
        self, data: dict[str, np.ndarray], padding: float = 0.05
    ...
    def _handle_infinities(self, values: np.ndarray, replacement: float = 0.0) -> np.ndarray:
...
class Base3DPlotter(BasePlotter):
    ...
    def _apply_3d_styling(
        self, ax, title=None, xlabel="Re", ylabel="Im", zlabel="|f(z)|", elev=30, azim=45
    ):
