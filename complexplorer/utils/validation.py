...
def validate_domain_or_mesh(
    domain: Any | None, mesh: np.ndarray | None, param_names: tuple = ("domain", "z")
) -> None:
...
def validate_function_or_values(
    func: Callable | None, values: np.ndarray | None, param_names: tuple = ("func", "f")
) -> None:
...
def validate_function(func: Any) -> Callable:
    ...
    if not callable(func):
        raise ValidationError(f"Function parameter must be callable, got {type(func).__name__}")
...
def validate_colormap(cmap: Any | None, default_class: Any = None) -> Any:
...
def validate_stl_parameters(
    size_mm: float, wall_thickness: float, min_size: float = 1.0, max_size: float = 500.0
) -> None:
    ...
    if size_mm < min_size or size_mm > max_size:
        raise ValidationError(f"Size must be between {min_size} and {max_size} mm, got {size_mm}")
...
def validate_array_shape(
    ...
    if not isinstance(array, np.ndarray):
        raise ValidationError(f"{param_name} must be a numpy array, got {type(array).__name__}")
...
def validate_file_extension(
    filename: str, valid_extensions: str | list, param_name: str = "filename"
) -> str:
    ...
    if not isinstance(filename, str):
        raise ValidationError(f"{param_name} must be a string, got {type(filename).__name__}")
...
def warn_deprecated(old_name: str, new_name: str, version: str = "next version") -> None:
    ...
    """
    warnings.warn(
        f"{old_name} is deprecated and will be removed in {version}. Use {new_name} instead.",
        DeprecationWarning,
        stacklevel=3,
    )
