...
class RectangularSphereGenerator:
    ...
    def generate(self) -> "pv.PolyData":
        ...
        if not HAS_PYVISTA:
            raise ImportError(
                "PyVista is required for mesh generation. Install with: pip install pyvista"
            )
...
def sphere_to_complex(
    x: np.ndarray, y: np.ndarray, z: np.ndarray, from_north: bool = True
) -> np.ndarray:
...
def complex_to_sphere(
    w: np.ndarray, to_north: bool = True
