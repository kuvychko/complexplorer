...
class Domain(ABC):
    ...
    def __init__(self, real: tuple[float, float], imag: tuple[float, float], square: bool = True):
    ...
    def mesh(self, n: int = 500) -> np.ndarray:
        ...
        real_axis = np.linspace(
            self.window_real[0], self.window_real[1], ceil(real_length / spacing)
        )
        imag_axis = np.linspace(
            self.window_imag[0], self.window_imag[1], ceil(imag_length / spacing)
        )
...
class Rectangle(Domain):
    ...
    def __init__(
        self, re_length: float, im_length: float, center: complex = 0 + 0j, square: bool = True
    ):
...
class Disk(Domain):
    ...
    def __init__(self, radius: float, center: complex = 0 + 0j):
...
class Annulus(Domain):
    ...
    def __init__(self, inner_radius: float, outer_radius: float, center: complex = 0 + 0j):
...
class CompositeDomain(Domain):
    ...
    def __init__(self, domain1: Domain, domain2: Domain, operation: str):
        ...
        if operation == "intersection":
            ...
            all_reals = [
                domain1.window_real[0],
                domain1.window_real[1],
                domain2.window_real[0],
                domain2.window_real[1],
            ]
            all_imags = [
                domain1.window_imag[0],
                domain1.window_imag[1],
                domain2.window_imag[0],
                domain2.window_imag[1],
            ]
    ...
    def mesh(self, n: int = 500, use_tight_bounds: bool = True) -> np.ndarray:
        ...
        if use_tight_bounds:
            ...
            real_axis = np.linspace(real_bounds[0], real_bounds[1], ceil(real_length / spacing))
            imag_axis = np.linspace(imag_bounds[0], imag_bounds[1], ceil(imag_length / spacing))
        else:
