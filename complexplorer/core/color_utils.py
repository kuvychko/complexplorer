"""Colour-space helpers for the perceptual colormap family.

Ported from complexplorer 2.0.0 (``complexplorer/core/color_utils.py``) so the colormaps that
shipped in that release render identically here: OkLCh/OKLab conversion, HSL, cubehelix, hue
interpolation and gamut clipping.

Internal module - these helpers are not part of the top-level public surface (they were not in
2.0.0's ``__all__`` either).
"""
...
def oklch_to_srgb(
    ...
    C: float | np.ndarray,
    H: float | np.ndarray,
    clip_method: str = "adaptive_chroma",
    ...
    if clip_method == "adaptive_chroma":
        ...
        raise ValidationError(
            f"Unknown clip_method: {clip_method!r}. Use 'adaptive_chroma' or 'simple'."
        )
...
def hsl_to_rgb(
    H: float | np.ndarray, S: float | np.ndarray, L: float | np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def cubehelix(
    h: float | np.ndarray, s: float = 0.5, r: float = -1.5, gamma: float = 1.0
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def interpolate_hue(
    h1: float, h2: float, t: float | np.ndarray, direction: str = "shortest"
) -> float | np.ndarray:
    ...
    if direction == "shortest":
        ...
        raise ValidationError(
            f"Unknown direction: {direction!r}. Use 'shortest', 'longest', 'clockwise' "
...
def clip_to_gamut(
    R: np.ndarray, G: np.ndarray, B: np.ndarray, preserve: str = "hue"
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if preserve == "hue":
        ...
        raise ValidationError(f"Unknown preserve mode: {preserve!r}. Use 'hue' or 'lightness'.")
