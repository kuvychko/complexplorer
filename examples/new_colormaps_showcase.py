...
def showcase_perceptual_colormaps():
    ...
    # PerceptualPastel
    cmap1 = cp.PerceptualPastel(phase_sectors=6, auto_scale_r=True)
...
def showcase_artistic_colormaps():
    ...
    cmap1 = cp.AnalogousWedge(H_center=0.55, H_wedge=0.2, phase_sectors=6, auto_scale_r=True)
    cp.plot(domain, f, cmap=cmap1, ax=axes[0,0], resolution=500)
    axes[0,0].set_title("AnalogousWedge\n(Ocean theme)")
    
    # AnalogousWedge - Sunset
    cmap2 = cp.AnalogousWedge(H_center=0.08, H_wedge=0.25, phase_sectors=6, auto_scale_r=True)
    cp.plot(domain, f, cmap=cmap2, ax=axes[0,1], resolution=500)
    axes[0,1].set_title("AnalogousWedge\n(Sunset theme)")
    
    # DivergingWarmCool
    cmap3 = cp.DivergingWarmCool(phase_sectors=6, auto_scale_r=True)
    cp.plot(domain, f, cmap=cmap3, ax=axes[0,2], resolution=500)
    axes[0,2].set_title("DivergingWarmCool\n(Cartographic)")
    
    # InkPaper
    cmap4 = cp.InkPaper(phase_strength=0.05, phase_sectors=8)
    cp.plot(domain, f, cmap=cmap4, ax=axes[0,3], resolution=500)
    axes[0,3].set_title("InkPaper\n(Minimalist)")
    
    # EarthTopographic
    cmap5 = cp.EarthTopographic(phase_sectors=6, auto_scale_r=True)
    cp.plot(domain, f, cmap=cmap5, ax=axes[1,0], resolution=500)
    axes[1,0].set_title("EarthTopographic\n(Terrain-inspired)")
    
    # FourQuadrant
    cmap6 = cp.FourQuadrant(phase_sectors=6, auto_scale_r=True)
...
def showcase_3d_with_new_colormaps():
    ...
    colormaps = [
        cp.PerceptualPastel(phase_sectors=6, auto_scale_r=True),
        cp.EarthTopographic(phase_sectors=6, auto_scale_r=True),
        cp.DivergingWarmCool(phase_sectors=6, auto_scale_r=True),
    ]
...
def interactive_parameter_exploration():
    ...
    for i, L in enumerate([0.4, 0.55, 0.7]):
        cmap = cp.PerceptualPastel(L_center=L, phase_sectors=6, auto_scale_r=True)
        cp.plot(domain, f, cmap=cmap, ax=axes[0,i], resolution=400)
        axes[0,i].set_title(f"L_center = {L}")
    
    # Vary Chroma
    for i, C in enumerate([0.05, 0.10, 0.15]):
        cmap = cp.PerceptualPastel(C=C, phase_sectors=6, auto_scale_r=True)
...
def save_gallery_images(save_path='examples/gallery/v2_colormaps'):
    ...
    colormaps = [
        ('perceptual_pastel', cp.PerceptualPastel(phase_sectors=6, auto_scale_r=True)),
        ...
        ('diverging_warm_cool', cp.DivergingWarmCool(phase_sectors=6, auto_scale_r=True)),
        ('isoluminant', cp.Isoluminant(L=0.6, C=0.15)),
        ('cubehelix_phase', cp.CubehelixPhase(phase_sectors=6, auto_scale_r=True)),
        ('ink_paper', cp.InkPaper(phase_strength=0.05)),
        ('earth_topographic', cp.EarthTopographic(phase_sectors=6, auto_scale_r=True)),
        ('four_quadrant', cp.FourQuadrant(phase_sectors=6, auto_scale_r=True)),
    ]
