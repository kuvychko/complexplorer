...
COLOR_SCHEMES = {
    "1": {"name": "Basic phase portrait", "cmap": cp.Phase(), "best_for": "zeros and poles"},
    "2": {
...
# Domain presets
DOMAINS = {
    "1": {"name": "Square (4x4)", "domain": cp.Rectangle(4, 4)},
    "2": {"name": "Wide rectangle (6x3)", "domain": cp.Rectangle(6, 3)},
    "3": {"name": "Disk (radius 2)", "domain": cp.Disk(2)},
    "4": {"name": "Annulus (0.5 to 2)", "domain": cp.Annulus(0.5, 2)},
    "5": {"name": "Large square (8x8)", "domain": cp.Rectangle(8, 8)},
    "6": {"name": "Disk with hole", "domain": cp.Disk(3) - cp.Disk(0.5)},
    "7": {
...
def stl_export(func_data, cmap):
    ...
    try:
        ...
        output_file = generator.generate_ornament(
            output_file=filename, size_mm=size_mm, smooth=True, verbose=True
        )
    ...
    for func_id in func_choices:
        ...
        if output_type in ["1", "3"]:
            ...
            png_file = os.path.join(
                output_dir, f"{func_id}_{func_data['name'].lower().replace(' ', '_')}.png"
            )
        ...
        if output_type in ["2", "3"]:
            ...
            try:
                ...
                stl_file = os.path.join(
                    output_dir, f"{func_id}_{func_data['name'].lower().replace(' ', '_')}.stl"
                )
                generator.generate_ornament(
                    output_file=stl_file, size_mm=80, smooth=True, verbose=False
                )
