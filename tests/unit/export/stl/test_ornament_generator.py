...
class TestOrnamentGenerator:
    ...
    def test_different_scaling_methods(self):
        ...
        assert np.allclose(radii_by_method["constant"], radii_by_method["constant"][0])
    ...
    def test_generate_and_save_pipeline(self, tmp_path):
        ...
        saved_path = gen.generate_and_save(
            str(filename), size_mm=60, repair=True, validate=True, verbose=False
        )
...
class TestCreateOrnament:
    ...
    def test_basic_creation(self, tmp_path):
        ...
        saved_path = create_ornament(func, str(filename), size_mm=40, resolution=50, verbose=False)
