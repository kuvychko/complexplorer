...
The transfer-functions capability is engineering mode's kernel (`cp.ee`): a rational
transfer-function object `H = N/D` whose poles, zeros, stability, and frequency response are
first-class, and whose instances are plain complex callables — so every complexplorer renderer
