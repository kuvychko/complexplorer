...
The expression capability provides a safe, PyVista-free evaluator that turns a
complex-function expression string into values over a complex grid `z`. It uses a curated,
complex-aware symbol table and restricts the grammar to portable math so expressions stay
safe to exchange across boundaries and portable to a native reimplementation.
