...
The surface-mesh capability is the backend-agnostic kernel for sampling complex functions and
building decorated 3D surfaces from those samples. It defines a `ComplexField` sampling container,
a unified `SurfaceMesh` wrapper around PyVista datasets, and the shared topology builders (planar
landscape and radial relief) that feed off the common modulus-scaling kernel and one decoration/export
path. The existing public PyVista plotting functions delegate to this kernel; this spec captures the
behavior the kernel and its callers must honor.
