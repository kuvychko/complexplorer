...
The matplotlib and PyVista Riemann renderers SHALL present the same conceptual visualization, while
each MAY differ in mesh construction, default resolution, default projection pole, interactivity,
and grid overlays.
...
- **THEN** both show a phase-colored sphere with equivalent modulus relief, differing only in performance, interactivity, and backend-specific options (such as the PyVista latitude/longitude grid overlay)
