...
- [x] 1.1 Create `examples/showcase.py` that imports `cp.catalog` and `generate_gallery`, and
...
- [x] 1.2 Render the 2D portraits + `index.json` by calling `generate_gallery()` into
  `examples/gallery/` (do not reimplement; reuse the deterministic library path)
- [x] 1.3 For each preset, render its tag-selected PyVista screenshots to
...
- [x] 1.4 Pick a high-res window size/dpi target and a fixed camera per render type so the
  bundle is visually consistent

## 1b. Colormap gallery (reference function × the real colormaps)

- [x] 1b.1 Render a designated reference preset's function (e.g. `rational_zeros_poles`) under the
...
- [x] 2.1 Write `examples/gallery/showcase.json`: catalog data per preset + every render's
...
- [x] 2.2 Ensure `showcase.json` is deterministic in structure (sorted keys, id-sorted presets,
  LF, trailing newline) even though the referenced images are best-effort

## 3. Generated docs gallery (middle path)

- [x] 3.1 Have `showcase.py` emit `docs/gallery/gallery.generated.md`: one entry per rendered
...
- [x] 3.2 Include a Colormaps section in the generated page (the reference function under each
  implemented colormap, with explicit-colormap snippets)
- [x] 3.3 Rewrite `docs/gallery/README.md` as a thin hand-written frame (intro, Wegert
...
- [x] 4.1 Run `python examples/showcase.py` locally; commit the regenerated
  `examples/gallery/<id>/*.png`, `showcase.json`, and `gallery.generated.md`
- [x] 4.2 Delete 24 of the 25 hand-named `examples/gallery/*.png`; KEEP the curated hero
  composite `Riemann_relief_map_20250726.png`
- [x] 4.3 Repoint `README.md`'s 3 gallery thumbnail links (lines ~101–103) to id-based renders;
  LEAVE the header hero image (line ~10) pointing at `Riemann_relief_map_20250726.png`
- [x] 4.4 Confirm no `.stl` is committed under `examples/gallery/`

## 5. Rebuild the modulus-scaling demo (PyVista)

- [x] 5.1 Add `examples/scripts/modulus_scaling_showcase.py` as a PyVista demo iterating the
...
- [x] 6.1 Add a non-render guard test that loads the committed `showcase.json` and asserts: it
...
- [x] 6.2 Extend the M1 examples-structure guard (or add a check) asserting no `.stl` under
...
- [x] 6.3 Run `pytest tests/` — full suite green
- [x] 6.4 Run `openspec validate rebuild-gallery-from-registry` and `openspec validate --specs`
  — clean
