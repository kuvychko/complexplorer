...
- [x] 2.1 New `mesh/riemann_surface.py`: a small family model (power, log) producing a
      structured parameter grid → embedded `X, Y, Z` → `pv.StructuredGrid`.
- [x] 2.2 `build_riemann_surface(family, *, n / turns, resolution) -> SurfaceMesh`: wrap the
      grid in `SurfaceMesh` and `attach_colors(cmap, w)` (phase of the value). Bypass
      `ComplexField`. PyVista-only.

## 3. PyVista renderer
- [x] 3.1 New `plotting/pyvista/riemann_surface.py`: `riemann_surface_pv(family, *, n=2 |
...
- [x] 3.2 Export `riemann_surface_pv` from the package public API (`__init__.py` + `__all__`).

## 4. Tests (PyVista-gated)
- [x] 4.1 `build_riemann_surface("power", n=2/3)`: point/face counts match the grid; `z ≈ w**n`
...
- [x] 4.2 `build_riemann_surface("log", turns=k)`: height spans `[0, 2πk]`; `(x,y)` periodic in `θ`.
- [x] 4.3 Colors: `RGB`/`phase` populated, finite, within `[0, 1]`.
- [x] 4.4 `riemann_surface_pv(..., return_plotter=True, interactive=False)` returns a plotter
      without rendering (Windows-CI offscreen-safe).

## 5. Docs
- [x] 5.1 README + CLAUDE.md: add `riemann_surface_pv` under the PyVista plot types; note the
      Riemann *surface* (multivalued cover) vs Riemann *sphere* (single-valued) distinction.

## 6. Close out
- [x] 6.1 `pytest tests/` green; `ruff` clean; `openspec validate --specs`.
- [x] 6.2 Update `openspec/ROADMAP.md` (status; note 3.0.0 PyPI release is now unblocked once
