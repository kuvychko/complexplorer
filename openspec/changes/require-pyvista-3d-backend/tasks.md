...
## 1. Dependency
- [ ] 1.1 `pyproject.toml`: move `pyvista>=0.47` from `[project.optional-dependencies]` into
...
- [ ] 3.2 Delete dead guards: `cli/main.py` `_need_pyvista()` + call sites;
      `export/stl/utils.py`, `plotting/pyvista/utils.py`, `utils/mesh.py` `if not HAS_PYVISTA`.

## 4. quick_plot fallback
- [ ] 4.1 `api.py`: drop the `plot_3d_landscape` / `plot_riemann` imports and the `use_pyvista`
...
## 5. CI
- [ ] 5.1 Collapse the workflow matrix `config: [base, pyvista]` to a single full-install
...
## 6. Tests
- [ ] 6.1 Delete `tests/unit/plotting/matplotlib/test_plot_3d.py`, `test_plot_3d_modulus.py`,
      `test_deprecation.py`.
- [ ] 6.2 Trim mpl-3D calls from `test_basic_workflows.py` / `test_end_to_end.py`.
- [ ] 6.3 Convert every `@skipif(not HAS_PYVISTA)` to unconditional; delete the inverse
...
- [ ] 6.4 Add a test: `quick_plot(mode="3d", backend="matplotlib")` raises the 3.0 error.

## 7. Version & docs
- [ ] 7.1 `_version.py` → `3.0.0` (PyPI release held for `add-riemann-surfaces`).
- [ ] 7.2 Update README / CLAUDE.md: drop removed functions and the optional-PyVista framing.

## 8. Close out
- [ ] 8.1 `pytest tests/` green; `ruff` clean; `openspec validate --specs`.
- [ ] 8.2 Update `openspec/ROADMAP.md` (status; note PyPI release bundled with riemann surfaces).
