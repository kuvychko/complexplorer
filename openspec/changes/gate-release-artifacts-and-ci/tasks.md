...
- [x] 1.1 Pin ruff to `>=0.16.7,<0.17` in the `dev` extra and in the CI lint job, so the gate
  cannot break on a new formatter release.
- [x] 1.2 Narrow `tool.ruff.extend-exclude` so `examples/**/*.py` is linted while notebooks stay
  exempt, and add the per-file `E402` exception for the scripts that must call
  `matplotlib.use("Agg")` before importing `pyplot`.
- [x] 1.3 Fix the findings this surfaces in `examples/`, and extend the CI lint job to cover
  `examples/`.

## 2. The distribution inspector

- [x] 2.1 Add `scripts/check_distribution.py`, which takes the built wheel and sdist and asserts:
...
- [x] 2.2 Make it runnable locally (`python scripts/check_distribution.py dist/`) and give it a
  clear failure report, since it is the tool someone runs before tagging.

## 3. The artifact gate in CI

- [x] 3.1 Add an `artifact` job: `uv build`, then `twine check dist/*`, then
  `scripts/check_distribution.py`.
- [x] 3.2 In the same job, create a throwaway environment, install the **wheel** from `dist/`, and
  ...
  - a small STL export
- [x] 3.3 Build a wheel from the sdist in a clean environment and import it.
- [x] 3.4 Print the installed footprint and a cold `import complexplorer` time for task 6.1.

## 4. Matrix

- [x] 4.1 Remove `continue-on-error` from Python 3.13; both 3.13 lanes already pass.
- [x] 4.2 Add a macOS lane on the current Python, as one explicit lane rather than a third
  dimension of the grid.
- [x] 4.3 Add an Ubuntu/3.11 minimum-dependency lane using
...
- [x] 4.4 Add a Python 3.14 lane that is allowed to fail, and leave the classifier off until it
  passes.
- [x] 4.5 Add a weekly scheduled run against the newest compatible dependencies.

## 5. Notebooks and the gallery contract

- [x] 5.1 Add a notebooks workflow running `pytest --nbmake examples/notebooks/` on a weekly
  schedule, on `workflow_dispatch` and on `v*` tags, with a headless display and a timeout.
- [x] 5.2 Add a test asserting that a freshly generated `index.json` is byte-identical to the
  committed one (images excluded, as the capability says).

## 6. Decisions and documentation

- [x] 6.1 Record the mandatory-PyVista decision in `docs/development/backend-policy.md` with the
  measured footprint and cold import time, and remove the residual optional-backend language there.
- [x] 6.2 Note in `CONTRIBUTING.md`'s absence — or in the closeout tracker until that file exists —
...
The wheel smoke test failed on its first real run, which is the gate doing its job. Both defects
are in shipped code, not in the harness, and both abort a command on a Windows console using a
legacy code page.

- [x] 8.1 `complexplorer list` raised `UnicodeEncodeError` on a stock `cmd.exe` (cp437) when
  printing the preset title `z³ - z`. Make the CLI degrade characters the console cannot represent
  instead of dying, keeping the notation in the catalog where it belongs.
- [x] 8.2 `OrnamentGenerator.generate_and_save` raised `UnicodeEncodeError` on a cp1252 stream
